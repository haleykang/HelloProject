...
public class TestMethodClass2 {
	...
	public static void main(String[] args) {
		...
		try {
			...
			System.out.print("2개의 정수와 하나의 연산자를 입력해주세요 : " );
			 ...
			 * 
			 */
			op1 = str_split[0].charAt(0); // 연산식이 앞에
			op2 = str_split[1].charAt(0); // 연산식이 중간에
			op3 = str_split[2].charAt(0); // 연산식이 맨 뒤에
			...
			if(op1 == '+' || op1 == '-' || op1 == '*' || op1 == '/' || op1 == '%') {
				...
				res = control_calc_method(a, op2, b);
				
				// 4. 계산 결과를 갖고 있는 변수 result의 값을 화면에 출력하기
				System.out.println(a + " " + op2 + " " + b + " " + "=" + " " + res);
			}
			else if (op3 == '+' || op3 == '-' || op3 == '*' || op3 == '/' || op3 == '%') {
				
				System.out.println("사용할 수 있는 사칙 연산 입니다.");
				System.out.println("위에서 만든 함수를 실행");
				
				a = Integer.parseInt(str_split[0]);
				b = Integer.parseInt(str_split[1]);
				
				// 3. 인픽스 표기법을 받는 함수를 실행해서 사칙 연산 하기
				
				res = control_calc_method(a, b, op3);
				
				// 4. 계산 결과를 갖고 있는 변수 result의 값을 화면에 출력하기
				System.out.println(a + " " + b + " " + op3 + " " + "=" + " " + res);
			}
				else {
